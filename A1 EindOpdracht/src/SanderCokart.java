public class SanderCokart {
	...
	public static double GetDeviation(int[] data) {// creation of a method to caculate the standard deviation
		...
		for (int i = 0; i < data2.length; i++) {// a loop that continues for as long as the array is. The array is
												// rowaverage but is redefined here as "data".
			average += data2[i];// average becomes the sum of all the data inside the data array
		}
		average /= data2.length;// average gets devided by the length of the data array
		for (int i = 0; i < data2.length; i++) {// loop that runs for the length fo the array
			data2[i] -= average;// data inside the array relative to the loop is subtracted by the value of
								// average
			data2[i] = data2[i] * data[i];// data inside the array relative to the loop equals itself squared
			deviation += data2[i];// deviation is the sum of the newly calculated data array
		}
		deviation /= data2.length;// deviation gets devided by the length of the array
